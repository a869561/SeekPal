...
export default function SourcesPage() {
  ...
  async function load() {
    try {
      ...
      toast.error("Error cargando fuentes");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleAdd(data) {
    try {
      ...
      toast.error(err.response?.data?.message || "Error añadiendo fuente");
    }
  }

  async function handleDelete(id) {
    if (!confirm("¿Eliminar esta fuente y todos sus ficheros indexados?")) return;
    try {
      await deleteSource(id);
      toast.success("Fuente eliminada");
      load();
    } catch {
      toast.error("Error eliminando fuente");
    }
  }
